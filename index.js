...
const findState = (clue) => {
    let foundState = null;

    if(!isNaN(clue)){
        foundState = states.find(s => Number(s.fips) === Number(clue));
    }
    else if(typeof clue === "string" && clue.length === 2){
        foundState = states.find(s => s.postal === clue.toUpperCase());
    } else if(typeof clue === "string" && !isNaN(clue[0])){
        foundState = states.find(s => s.admissionOrder.toLowerCase() === clue.toLowerCase());
    }
    else {
        foundState = states.find(s => s.name.toLowerCase() === clue.toLowerCase())
    }
